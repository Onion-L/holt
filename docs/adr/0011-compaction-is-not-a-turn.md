...
- `/compact` while the execution channel is occupied is refused. Ordinary
  messages now queue; slash-command queueing follows in message-queue ticket
  04. Automatic and manual Compaction share the execution lock and receive
  the current cancellation token. Stop pauses pending ordinary messages;
  a manual Compaction failure also pauses them. `/compact` with nothing outside the recent tail
  reports that there is nothing to compact and makes no request. A failed
