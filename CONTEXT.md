...
- **Slash command**: a composer directive starting with `/` that the UI intercepts and handles itself — sent as a typed command (`/skill`, `/compact`), answered locally (`/plan`), or rewritten to a bundled prompt (`/init`); the raw directive never reaches the model.
