...
- **Last-message edit**: a revision of the latest user message already in a Chat's Transcript, replacing that message and its subsequent conversation with a new Turn. Drafting the revision leaves an active Turn running; submitting it requests the replacement.
- **Pending message**: a user message accepted for later execution in its chat; skill invocations ride inside one as mentions. Each pending message starts its own Turn when its turn to run arrives.
...
- **Slash command**: a composer directive starting with `/` that the UI intercepts and handles itself — sent as a typed command (`/compact`), answered locally (`/plan`), or rewritten to a bundled prompt (`/init`); the raw directive never reaches the model. Skills are not a directive — they are inline mentions (see Skill mention).
...
- **Skill mention**: a `$name` token or a linked `[$name](SKILL.md path)` reference written inline in a user message. Resolution is path-first — the linked form invokes exactly the file it points at, even a shadowed one — falling back to the name; an unresolved mention stays ordinary text. The `/` menu inserts the linked form as a `$name` chip.
  _Avoid_: slash invocation, `/skill` command
- **Skill invocation**: handing a skill's full content to the agent. The model does it itself by reading the skill file once the listing matches its task; the user forces it with a Skill mention inline in an ordinary message, and the engine prepends the skill's formatted block to that Turn's prompt.
- **Transcript**: the scrollable rendered history of a chat — user messages, agent output, and tool rows in order. Compaction never removes rows from it; a Last-message edit replaces the latest user message and removes the subsequent conversation. It is anchored document-style: rows lay out from the top of the pane, and a short transcript leaves empty space below rather than rising from the bottom.
...
- **Compaction**: replacing the older part of an agent's History with a model-written summary while keeping a recent tail verbatim. Triggered automatically when the History approaches the model's context window, or when the provider rejects a request as too long (the Turn then continues); a chat also supports manual `/compact`. Marked in the Transcript by a divider whose summary can be expanded.
