...
- **Interrupt confirmation**: the two-press protocol for interrupting a live Turn from the keyboard: the first press arms a pending interrupt, and a second press within a short window — or acting on the armed control — confirms it. An armed confirmation lapses when the window passes, the Turn ends, or the chat changes. Guards against accidental interrupts; the mouse stop control stays immediate.
