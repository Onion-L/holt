...
use holt_proto::{GitHistoryCommit, GitHistoryPage, GitHistoryRef, GitHistoryRefKind};
...
impl GitHistory {
    ...
    fn context(&self, cx: &App) -> Option<(String, String)> {
    ...
    pub fn ensure_loaded(&mut self, cx: &mut Context<Self>) {
        self.started = true;
        let Some((key, cwd)) = self.context(cx) else {
    ...
    fn load_older(&mut self, cx: &mut Context<Self>) {
        ...
        self.fetch_page(key, cwd, cursor, false, cx);
    }
            ...
            this.update(cx, |history, cx| {
                ...
                match result.and_then(|value| {
                    serde_json::from_value::<GitHistoryPage>(value)
                        .map_err(|error| holt_rpc::RpcError::Failed(error.to_string()))
                }) {
                    Ok(page) => {
