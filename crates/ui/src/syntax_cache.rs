...
use holt_syntax::{HighlightedDocument, LanguageId};
    ...
    std::mem::size_of::<HighlightedDocument>()
        .saturating_add(
            document
                .lines
                .capacity()
                .saturating_mul(std::mem::size_of::<Vec<holt_syntax::HighlightSpan>>()),
        )
        .saturating_add(document.lines.iter().fold(0usize, |total, line| {
            total.saturating_add(
                line.capacity()
                    .saturating_mul(std::mem::size_of::<holt_syntax::HighlightSpan>()),
            )
        }))
}
