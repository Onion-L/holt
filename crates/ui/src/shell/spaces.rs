...
//! the filtered Sessions list, and the add-space palette (⌘K-style folder
//! browser + Locations rail).
//!
//! A space = a synced project folder on this machine. The dropdown only
//! FILTERS the sidebar's session list (never the tab strip) and hosts space
...
/// across the top, folder browser on the left, a Locations rail on the right,
/// kbd-hint footer. One surface — picking a drive in the rail rebrowses in
...
pub(super) struct AddSpaceFlow {
    ...
    /// This machine's mounted drives/volumes (the rail's Locations rows).
    /// Best-effort: an error just leaves the section at Home only.
    drives: Loadable<Vec<DriveEntry>>,
    /// Requested browser path (`None` = home).
    browser_path: Option<String>,
    /// The local home dir (the path a `None` browse resolved to) —
    /// breadcrumbs fold everything up to here into the home crumb.
    home: Option<String>,
    /// Best-effort git seed for the CURRENT browser path (known when we
    /// descended through an entry whose `is_repo` we saw; the engine's
...
impl Shell {
    ...
    fn render_spaces_menu(&mut self, theme: &Theme, cx: &mut Context<Self>) -> AnyElement {
        ...
        let rows = self.spaces_menu_rows(cx);
        let filter = self.settings.space_filter.clone();
        let details: Vec<(SpacesMenuRow, SharedString)> = {
            let state = self.state.read(cx);
            rows.iter()
                .map(|row| match row {
                    SpacesMenuRow::All => (row.clone(), SharedString::from("All projects")),
                    SpacesMenuRow::Space(id) => match state.space_row(id) {
                        Some(space) => (row.clone(), space.display_name().to_string().into()),
                        None => (row.clone(), SharedString::from("?")),
                    },
                    SpacesMenuRow::AddSpace => (row.clone(), SharedString::from("New project…")),
                })
                .collect()
        };
        ...
        let list = div()
            ...
            .children(details.into_iter().enumerate().map(|(ix, (row, label))| {
    ...
    /// draws them — the user's sort applied. The jump shortcuts and session
    ...
    pub(super) fn render_active_rows(
        ...
        for (group, rows) in groups {
            ...
            let collapse_key = format!("list:{key}");
    ...
    pub(super) fn render_archived_section(
        ...
        let now = Utc::now();
        let filter = self.settings.space_filter.clone();
        let mut rows: Vec<holt_proto::Chat> = {
            let state = self.state.read(cx);
            state
                .chats
                .iter()
    ...
    /// ListDrives on the local engine. Failures stay silent — the section
    /// just shows Home; the folder browser's own error row already covers
    ...
    fn load_space_drives(&mut self, cx: &mut Context<Self>) {
        ...
        flow.drives_task = Some(cx.spawn(async move |this, cx| {
            let result = engine
                .client()
                .call(methods::LIST_DRIVES, serde_json::json!({}))
                .await;
    ...
    fn add_space_filtered(&self, cx: &App) -> Vec<holt_proto::FolderEntry> {
    ...
    fn submit_add_space(&mut self, cx: &mut Context<Self>) {
        ...
        // The same folder already has a space → just switch to it. The
    ...
    /// breadcrumbs + folder list beside the Locations rail · kbd-hint footer.
    pub(super) fn render_add_space_overlay(
        ...
        // The drive whose mount folds into a breadcrumb (like the home dir
        // folds into the home crumb). System "/" keeps the plain full-path
        // crumbs.
        ...
        // ── breadcrumbs ("~ / Projects / holt"): the quiet mono path voice,
        //    `/` separators. The home crumb stands in for the home dir —
        //    everything up to the resolved home path folds into it; below
        //    home the full path shows. Ancestors (home crumb included) are
        //    clickable.
        ...
        let list: AnyElement = if loading {
            ...
            // as the engine being unreachable. The engine's folder errors all
            // name the folder ("could not read that folder: …").
            let engine_line = if message.contains("folder") {
                message
            } else {
                "The engine didn't respond — is it running?".to_string()
            };
            popover::error_row(&theme, &engine_line)
                .px(px(14.0))
                .py(px(10.0))
                .child(
                    div()
        ...
        // ── rail: Locations (home + this machine's mounted drives). Rows are
        //    the tab recipe (h-28 rounded-8 washes), vertical.
        ...
        let rail = div()
            ...
            .overflow_y_scroll()
            // Clicking a row rebrowses the palette at that root; the row
            // owning the browsed path carries the selection wash. Drives
            // arrive best-effort (ListDrives) — until then the section is
            // just Home, and on error it stays that way.
            .child(
                div()
                    .px(px(8.0))
                    .pt(px(2.0))
        ...
        // ── body: folder column (crumbs + list) beside the Locations rail.
