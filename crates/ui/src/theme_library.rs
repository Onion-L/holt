...
use gpui::{App, Global};
use holt_theme::vscode::SourceCompilation;
use holt_theme::{
...
fn reconcile_and_refresh(cx: &mut App) {
    ...
    for appearance_kind in [Appearance::Light, Appearance::Dark] {
        let model = if appearance_kind.is_light() {
            holt_theme::Appearance::Light
        } else {
            holt_theme::Appearance::Dark
        };
...
mod tests {
    ...
    fn failed_persistence_leaves_library_state_unchanged() {
        ...
        next.entries.push(CustomThemeEntry {
            ...
            source: holt_theme::CustomThemeSource::ImportedSnapshot {
                imported_from: None,
            },
            family: holt_theme::ThemeFamily {
                id: "unpersisted".into(),
                name: "Unpersisted".into(),
                variants: Vec::new(),
            },
            ...
            status: holt_theme::CustomThemeStatus::Ready,
        });
