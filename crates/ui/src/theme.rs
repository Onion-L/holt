...
//! lightness steps; the same scale holt's Tailwind theme used) into gpui [`Hsla`].
...
pub enum AccentColor {
    /// The exact upstream Holt indigo.
    #[default]
...
impl AccentColor {
    ...
    fn tokens(self, appearance: Appearance) -> AccentTokens {
        ...
        let (primary, strong) = match (self, appearance) {
            (Self::Holt, Appearance::Dark) => {
                (oklch(0.673, 0.182, 276.935), oklch(0.585, 0.233, 277.117))
            }
            (Self::Holt, Appearance::Light) => {
...
fn model_appearance(appearance: holt_theme::Appearance) -> Appearance {
    match appearance {
        holt_theme::Appearance::Dark => Appearance::Dark,
        holt_theme::Appearance::Light => Appearance::Light,
    }
}
...
impl TerminalColors {
    ...
    fn holt(appearance: Appearance) -> Self {
        let id = match appearance {
            Appearance::Dark => "holt-dark",
            Appearance::Light => "holt-light",
        };
        let registry = ThemeRegistry::active();
        Self::from_variant(registry.variant(id).expect("Holt terminal palette exists"))
    }
}
...
impl Theme {
    ...
    /// Main-panel header height (holt `h-11`) — in-card headers (changes pane).
    ...
    /// Reserved status strip under the content outlet (holt `h-6`) — the
    ...
    /// fixed Holt grey would erase that palette's identity.
    ...
    /// Increase tint coverage only as far as needed for Holt's shared text
    ...
    /// theme, so forcing frost does not reintroduce Holt's neutral hover.
    ...
    pub fn dark_with_accent(accent_color: AccentColor) -> Self {
        ...
        Self {
            ...
            terminal: TerminalColors::holt(Appearance::Dark),
    ...
    pub fn light_with_accent(accent_color: AccentColor) -> Self {
        ...
        Self {
            ...
            terminal: TerminalColors::holt(Appearance::Light),
            font_sans: "Geist".into(),
            font_sans_fixed: "Geist".into(),
            font_mono: "Geist Mono".into(),
            font_sans_fallback: system_sans().into(),
    ...
    pub fn for_selection(
        ...
        let variant = registry
            ...
            .expect("the built-in registry contains both Holt appearances");
    ...
    pub(crate) fn from_variant(
        ...
        let accent_color = match accent_selection {
            AccentSelection::ThemeDefault => AccentColor::Holt,
...
mod tests {
    ...
    fn neutral_950_is_0a0a0a() {
        // oklch(0.145 0 0) is Tailwind neutral-950, holt's app background.
    ...
    #[test]
    fn holt_accent_is_the_exact_upstream_default() {
        let dark = Theme::dark();
        let light = Theme::light();
        assert_eq!(dark.accent_color, AccentColor::Holt);
    ...
    fn theme_recommendation_and_surface_override_resolve_independently() {
        ...
        let holt = Theme::dark();
        assert_ne!(catppuccin.surface, holt.surface);
        ...
        let opaque_holt = Theme::for_selection(
            Appearance::Dark,
            "holt-dark",
            AccentSelection::ThemeDefault,
            SurfacePreference::Opaque,
        );
        assert_eq!(opaque_holt.surface_treatment, SurfaceTreatment::Opaque);
        assert_eq!(opaque_holt.glass(), opaque_holt.surface);
    }
