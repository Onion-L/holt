...
//! the chunked upload to the engine, the plain-text attachment-ref
//! transport that rides the prompt, the transcript read-back cache, and the
//! full-size preview lightbox.
//!
//! Ports of holt's `composer/use-attachments.ts` (staging/upload),
...
/// Base64 chars per `UploadChunk`. The size descends from the retired relay's
/// hard ceiling (a WebSocket message capped at 1 MiB, one chunk per frame with
/// a JSON envelope + uleb header of ~150 bytes) — 680 000 chars ≈ 510 KB
/// binary leaves ~35% headroom, still a sane frame budget in-process. Multiple
/// of 4 so a slice of the whole-file base64 stays independently decodable.
/// The old 60 000 (45 KB) made a 3 MB screenshot ~70 sequential round trips —
/// each one a stall opportunity on a flaky link.
...
/// Per-call deadlines (desktop state.ts): a stalled-but-open RPC never fails
/// on its own, so every attachment call races a timer. The first chunk gets
/// 90s (the engine may still be cold), later chunks 30s; commit 150s (it must
/// outlast the engine's server-side assemble); reads 20s.
...
/// `UploadCommit{uploadId,fileName}` → the durable absolute path. The caller
/// mints `upload_id` — the queued-attachment flow derives its `pending://`
/// refs from the same identity before the bytes move. `progress` (when
/// given) accumulates uploaded BINARY bytes — the composer's "Uploading… N%"
/// reads it every paint. Errors return the raw cause (the composer shows
/// friendly copy).
        ...
        futures::stream::iter(ranges.iter().cloned().map(Ok::<_, String>))
            .try_for_each_concurrent(UPLOAD_CONCURRENCY, |(seq, range)| {
                ...
                async move {
                    ...
                    // The first WINDOW (not just seq 0) gets the cold-start
                    // allowance — its chunks all start before the pipe is warm.
...
/// A transcript image read back from the engine.
...
/// the same stuck-offset guard as holt's `readAttachmentImage`).
...
mod tests {
    ...
    #[test]
    fn upload_chunk_stays_within_a_frame_budget() {
