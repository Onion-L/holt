...
//!   the banner is attributed to Holt with its icon. The API is deprecated
...
//!   center — those adopt the installed Holt app's bundle identity (the
//!   terminal-notifier technique: override `-[NSBundle bundleIdentifier]` for
//!   the main bundle), so dev banners still show the Holt icon; a click may
//!   focus the installed app rather than the dev binary — acceptable for a
//!   dev-only path. Machines without Holt installed fall back to
...
//! - `HOLT_DISABLE_NOTIFICATIONS` env kill-switch + the
...
const DISABLE_ENV: &str = "HOLT_DISABLE_NOTIFICATIONS";
...
/// which the center resolves to its name + icon.
#[cfg(target_os = "macos")]
const MACOS_BUNDLE_ID: &std::ffi::CStr = c"sh.holt.app";
...
mod delegate {
    ...
    pub(super) fn always_present() -> *mut Object {
        ...
        *DELEGATE.get_or_init(|| unsafe {
            let mut decl = ClassDecl::new("HoltNotifyDelegate", class!(NSObject))
                .expect("HoltNotifyDelegate registered twice");
            decl.add_method(
...
/// `-[NSBundle bundleIdentifier]` to answer the installed Holt app's id for
