//! Animation kit — the holt motion catalog as reusable helpers over gpui
...
//! - `holt-pulse` 2.4s staggered cell opacity 0.08→1, scale 0.9→1 (loaders)
...
/// holt's signature entrance curve — CSS `cubic-bezier(0.16, 1, 0.3, 1)`.
...
/// (`transition-colors` et al. carry it unless overridden; holt never does).
...
/// (holt also scales 0.96→1; divs have no scale transform in gpui — approximated.)
...
/// Holt-pulse floor opacity.
// The loader constants and math live in `holt_proto::motion` (pure phase
...
// the pointer enters. The original holt puts Tailwind `transition-colors`
...
/// Dev/measurement knob (`HOLT_MOTION_SCALE`, default 1): stretches every
/// catalog timeline by this factor — e.g. `HOLT_MOTION_SCALE=10` slows the
