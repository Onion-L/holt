...
//! collapse flags (holt persisted the same set in localStorage).
//!
...
pub enum SidebarOrganization {
    ...
    ByProject,
    /// Legacy persisted value. Device scope went away with multi-device and
    /// is normalized to [`Self::InOneList`] on load.
    ByDevice,
    #[default]
    InOneList,
}
...
impl ShortcutId {
    ...
    /// Row label (holt lib/shortcuts.ts `SHORTCUT_DEFINITIONS`, verbatim).
...
impl UiSettings {
    ...
    fn migrated(mut self) -> Self {
        if self.accent == holt_theme::AccentSelection::ThemeDefault
...
mod tests {
    ...
    #[test]
    fn legacy_device_organization_normalizes_to_one_list() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::write(
            UiSettings::path(dir.path()),
            r#"{"sidebarOrganization":"byDevice"}"#,
        )
        .unwrap();
    ...
    fn legacy_accent_color_migrates_to_an_explicit_preset() {
        ...
        assert_eq!(
            loaded.accent,
            holt_theme::AccentSelection::Preset(holt_theme::AccentPreset::Cyan)
        );
