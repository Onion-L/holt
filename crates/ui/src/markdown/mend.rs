...
//! `remend`, ported to holt's incremental parser).
//!
...
//! handlers don't apply here — holt renders raw HTML literally and has no
//! math):
...
pub const PENDING_LINK_URL: &str = "holt:pending-link";
...
pub fn close_hanging(text: &str) -> Option<String> {
    ...
    pending.sort_by_key(|b| std::cmp::Reverse(b.0));
...
mod tests {
    ...
    fn links_mend_to_pending_sentinel() {
        mends("[docs](https://x.dev/lo", "[docs](holt:pending-link)");
        mends("[docs](", "[docs](holt:pending-link)");
        mends("see [do", "see [do](holt:pending-link)");
        mends("![alt](https://x/i.p", "![alt](holt:pending-link)");
        stays("see [");
    ...
    fn link_urls_allow_nested_parens() {
        ...
        mends("[a](https://x.dev/(y", "[a](holt:pending-link)");
    }

    #[test]
    fn emphasis_inside_link_text_closes_inside() {
        mends("[**a", "[**a**](holt:pending-link)");
        mends("**a [b", "**a [b](holt:pending-link)**");
    }
