...
/// Gap between markdown blocks inside one message (holt mdBlockGap).
pub const MD_BLOCK_GAP: f32 = 12.0;
/// Body text size / line height (holt: 14px / 22px).
...
// Table metrics — a port of mugen-markdown 0.6.2's `TableBlock` under holt's
...
/// Uniform cell padding in px (holt `table.cellPadding`).
pub const TABLE_CELL_PADDING: f32 = 12.0;
/// Hairline between rows in px (holt `table.gap`).
pub const TABLE_DIVIDER: f32 = 1.0;
/// Header row font weight (holt `table.headerWeight` = 700).
...
/// Minimum rendered column width in px, padding included (holt
...
/// Tight monochrome heading scale (holt: h2 ≈ 16px semibold; headings step
...
/// A GFM table — a port of mugen-markdown's `TableBlock` under holt's md
...
/// holt's `table.headerWeight`; strong runs never drop below semibold).
fn flatten_runs_weighted(runs: &[InlineRun], theme: &Theme, base_weight: FontWeight) -> FlatText {
    ...
    for run in runs {
        ...
        // Links stay monochrome — foreground with an underline (holt's md
