...
fn parse_started_block(cur: &mut Cursor) -> Vec<Block> {
    ...
    match tag {
        ...
        Tag::HtmlBlock => {
            // Render raw HTML blocks as plain text (holt's markdown does the same).
...
mod tests {
    ...
    fn bare_urls_autolink() {
        assert_eq!(
            only_link("PR is updated: https://github.com/holtsh/holt/pull/31\n"),
            Some((
                "https://github.com/holtsh/holt/pull/31".into(),
                "https://github.com/holtsh/holt/pull/31".into()
            ))
        );
        assert_eq!(
