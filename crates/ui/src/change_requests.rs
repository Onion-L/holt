...
use holt_proto::{ChangeRequestSummary, Chat, CheckoutChangeRequestStatus, Space};
...
impl ChangeRequestBadgeModel {
    pub fn from_summary(summary: &ChangeRequestSummary) -> Self {
        use holt_proto::ChangeRequestState;
...
/// One engine-side checkout watch. Multiple chats can share this target.
...
pub(crate) struct ChangeRequestClientState {
    ...
    /// Devices whose engine rejected this versioned capability. The engine is
    /// in-process, so a rejection cannot be invalidated by a device upgrade.
...
pub(crate) fn watch_params(target: &ChangeRequestWatchKey) -> serde_json::Value {
    serde_json::json!({
        "cwd": target.cwd,
...
mod tests {
    ...
    fn with_source(mut chat: Chat, branch: &str) -> Chat {
        chat.source_context = Some(holt_proto::ConversationSourceContext {
            checkout_id: "checkout".into(),
            repo_root: "/repo".into(),
            cwd: "/repo".into(),
            branch: branch.into(),
    ...
    #[test]
    fn watch_params_never_carry_a_relay_target() {
