//! A retained terminal viewport and its RPC session.
//!
...
use gpui::{
    App, Context, Entity, FocusHandle, Focusable, IntoElement, KeyBinding, KeyDownEvent,
...
pub use super::panel::ToggleTerminal;

pub struct PaneFocused;
impl gpui::EventEmitter<PaneFocused> for TerminalPane {}
...
pub struct TerminalPane {
    ...
    search: Entity<crate::composer::ComposerInput>,
    search_open: bool,
    search_match: bool,
    search_sub: Option<Subscription>,
    mouse_cell: Option<(usize, usize)>,
    focus_subs: Vec<Subscription>,
...
impl TerminalPane {
    pub fn new(state: Entity<AppState>, chat_id: String, cx: &mut Context<Self>) -> Self {
        ...
        Self {
            ...
            search: cx.new(|cx| crate::composer::ComposerInput::new("Find in terminal", cx)),
            search_open: false,
            search_match: true,
            search_sub: None,
            mouse_cell: None,
    ...
    pub fn new_embedded(state: Entity<AppState>, chat_id: String, cx: &mut Context<Self>) -> Self {
        let mut panel = Self::new(state, chat_id, cx);
        panel.search_sub = Some(
            cx.subscribe(&panel.search, |this, _, event, cx| match event {
                crate::composer::ComposerInputEvent::Edited => this.find(false, true, cx),
                crate::composer::ComposerInputEvent::Submitted => this.find(false, false, cx),
                _ => {}
            }),
        );
        panel.ensure_tab(cx);
    ...
    pub fn close_session(&mut self, cx: &mut Context<Self>) {
        ...
        for (_, tabs) in self.chats.drain() {
            for mut tab in tabs.tabs {
                if let Some(run) = tab._run.take() {
                    run.detach();
                }
                if let (Some(engine), Some(id)) = (engine.clone(), tab.terminal_id) {
    ...
    fn restart(&mut self, cx: &mut Context<Self>) {
        ...
        self.open_tab(self.chat_id.clone(), cx);
    }
    ...
    fn find(&mut self, backwards: bool, reset: bool, cx: &mut Context<Self>) {
        let query = self.search.read(cx).text().to_owned();
        self.search_match = self
            .with_active_emulator(cx, |e| e.find(&query, backwards, reset))
            .unwrap_or(false);
        cx.notify();
    }
    fn report_focus(&mut self, focused: bool, cx: &mut Context<Self>) {
        ...
        if self
            .active_tab(cx)
            .is_some_and(|t| t.emulator.focus_reporting())
        {
            self.queue_input(if focused { b"\x1b[I" } else { b"\x1b[O" }, cx);
        }
        cx.notify();
    }
    ...
    fn report_mouse(
        ...
        let bytes = self.active_tab(cx).and_then(|t| {
            t.emulator.mouse_bytes(
                code,
                released,
                motion,
                hit.row,
                hit.col,
                (mods.alt as u8) * 8 + (mods.control as u8) * 16,
            )
        });
        let Some(bytes) = bytes else {
            return false;
        };
        if !motion || self.mouse_cell != Some((hit.row, hit.col)) {
            self.queue_input(&bytes, cx);
        }
        self.mouse_cell = Some((hit.row, hit.col));
        cx.stop_propagation();
        true
    }
    ...
    fn spawn_session(
        ...
        cx.spawn(async move |this, cx| {
            ...
            let attached = this
                .update(cx, |panel, cx| {
                    if let Some(tab) = panel.tab_mut(&chat, key) {
                        ...
                        cx.notify();
                        Some((tab.emulator.cols(), tab.emulator.rows()))
                    } else {
                        None
                    }
                })
                .unwrap_or(None);
            let Some((cols, rows)) = attached else {
            ...
            // The first layout may finish while OpenTerminal is still pending.
            // Apply its current size even if the debounced resize already ran.
            let _ = engine.client().call(
            ...
            loop {
                ...
                while let Some(value) = rx.recv().await {
                    ...
                    attempt = 0;
                    if let TerminalEvent::Data { data, .. } = &event { frame_bytes += data.len(); }
    ...
    fn apply_stream_event(
        ...
        match event {
            TerminalEvent::Gap { seq } => {
                self.stream_notice =
                    Some("Output was lost while disconnected; this screen is incomplete.".into());
                let Some(tab) = self.tab_mut(chat, key) else {
                    return StreamDisposition::Stop;
                };
                tab.last_seq = seq;
                tab.emulator =
                    Emulator::new(tab.emulator.cols() as u16, tab.emulator.rows() as u16);
                tab.emulator
                    .feed(b"[Output was lost while disconnected; this screen is incomplete.]\r\n");
                cx.notify();
    ...
    fn flush_input(&mut self, chat: String, key: u64, cx: &mut Context<Self>) {
        ...
        let bytes = tab.coalescer.take();
        let previous = tab.write_task.take();
        tab.write_task = Some(cx.spawn(async move |this, cx| {
            ...
            for chunk in bytes.chunks(64 * 1024) {
                if let Err(error) = engine
                    ...
                    .call(
                        methods::WRITE_TERMINAL,
                        serde_json::json!({ "terminalId": id, "data": encode_base64(chunk) }),
                    )
                    .await
                {
                    let _ = this.update(cx, |pane, cx| {
                        pane.stream_notice = Some(format!("Terminal input failed: {error}"));
                        cx.notify();
                    });
                    break;
                }
    ...
    fn on_key_down(&mut self, event: &KeyDownEvent, _window: &mut Window, cx: &mut Context<Self>) {
        ...
        if !self.marked.is_empty() && !mods.platform {
            return;
        }
        ...
        let app_cursor = self
            ...
            .unwrap_or(false);
        // macOS also sets key_char for Enter and Tab. Only printable text
        // belongs to the text input handler; control keys need PTY encoding.
        if !mods.control
            && !mods.alt
            && !mods.platform
            && (ks
    ...
    fn on_mouse_down(
        ...
        if event.modifiers.platform
            && let Some(url) = self.with_active_emulator(cx, |e| e.url_at(point)).flatten()
        {
            cx.open_url(&url);
            return;
        }
    ...
    fn render_tab_bar(&mut self, chat: &str, cx: &mut Context<Self>) -> impl IntoElement + use<> {
        ...
        // Holt terminal-panel.tsx: `flex h-10 items-center border-b
        ...
        div()
            ...
            .children(
                tab_elements
                    ...
                    .map(|(ix, key, title, selected, exited)| {
                        ...
                        // Holt tab: `h-7 rounded-lg pl-2 pr-1 gap-1.5 text-xs`,
                        ...
                        let tab_el = div()
                            ...
                            .pr(px(4.0))
                            .rounded(px(8.0))
                            // holt terminal-panel.tsx tab: `transition-colors`.
            ...
            .child(
                div()
                    ...
                    .rounded(px(8.0))
                    .cursor_pointer()
                    // holt terminal-panel.tsx icon buttons: `transition-colors`.
                    .bg(motion::hover_blend(
                        "term-new-tab",
                        gpui::transparent_black(),
            ...
            // Collapse chevron pinned right (holt "Hide terminal" ⌘J).
            .child(div().flex_1())
            .child(
                div()
...
impl gpui::EntityInputHandler for TerminalPane {
    fn text_for_range(
        ...
        let units: Vec<_> = self.marked.encode_utf16().collect();
    ...
    fn replace_and_mark_text_in_range(
        ...
        let mut units: Vec<_> = self.marked.encode_utf16().collect();
        ...
        self.marked = String::from_utf16_lossy(&units);
        self.marked_selection = selection
            .map(|r| (start + r.start).min(units.len())..(start + r.end).min(units.len()))
            .unwrap_or(units.len()..units.len());
        cx.notify();
    }
    fn bounds_for_range(
        &mut self,
        ...
        Some(gpui::Bounds::new(
            gpui::point(
                g.origin.x + px(cursor.col as f32 * g.cell_w),
                g.origin.y + px(cursor.row as f32 * g.line_h),
            ),
            gpui::size(px(g.cell_w), px(g.line_h)),
        ))
    }
...
impl Drop for TerminalPane {
    fn drop(&mut self) {
        // An in-flight OpenTerminal must finish so its unattached-session
        // branch can release the PTY even after the window has disappeared.
        for tabs in self.chats.values_mut() {
...
impl Render for TerminalPane {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        if self.focus_subs.is_empty() {
            self.focus_subs
                .push(cx.on_focus(&self.focus_handle, window, |this, _, cx| {
                    this.report_focus(true, cx)
                }));
            self.focus_subs
                .push(cx.on_blur(&self.focus_handle, window, |this, _, cx| {
        ...
        div()
            ...
            .when_some(panel_bg, |el, bg| el.bg(bg))
            .children(tab_bar)
            .when_some(self.stream_notice.clone(), |d, notice| {
                d.child(
                    div()
                        .px_2()
                        .py_1()
                ...
                d.child(
                    div()
                        ...
                        .on_action(cx.listener(
                            |this, _: &crate::composer::MentionEscape, window, cx| {
                        ...
                        .when(!self.search_match, |d| {
                            d.child(div().text_color(theme.text_muted).child("No matches"))
                        })
                        .child(
                            super::panel::tool(
                                "terminal-find-prev",
                            ...
                            .on_click(cx.listener(|this, _, _, cx| this.find(true, false, cx))),
                        )
                        .child(
                            super::panel::tool(
                                "terminal-find-next",
                            ...
                            .on_click(cx.listener(|this, _, _, cx| this.find(false, false, cx))),
                        )
                        .child(
                            super::panel::tool(
                                "terminal-find-close",
            ...
            .child(
                div()
                    ...
                    .on_action(cx.listener(|this, _: &crate::composer::Copy, _, cx| {
                        this.copy_selection(cx);
                    }))
                    .on_action(cx.listener(|this, _: &crate::composer::Paste, _, cx| {
                    ...
                    .on_scroll_wheel(cx.listener(|this, event: &gpui::ScrollWheelEvent, _, cx| {
                        ...
                        if !event.modifiers.shift {
                            ...
                            let bytes = geometry.and_then(|g| {
                                ...
                                this.active_tab(cx).and_then(|t| {
                                    t.emulator.mouse_bytes(
                                        if step > 0 { 64 } else { 65 },
                                        false,
                                        false,
                                        hit.row,
                                        hit.col,
                            ...
                            if let Some(bytes) = bytes {
                                for _ in 0..step.unsigned_abs().min(32) {
                                    this.queue_input(&bytes, cx);
                                }
                                return;
                            }
                            if this
                                .active_tab(cx)
                                .is_some_and(|t| t.emulator.alternate_screen())
                            {
                                let key = if step > 0 { "up" } else { "down" };
                                let app = this
                                    .active_tab(cx)
                                    .is_some_and(|t| t.emulator.app_cursor_mode());
                                if let Some(bytes) =
                                    keystroke_bytes(key, None, &gpui::Modifiers::default(), app)
                                {
                                    for _ in 0..step.unsigned_abs().min(32) {
                                        this.queue_input(&bytes, cx);
                                    }
...
mod tests {
    ...
    #[gpui::test]
    fn terminal_macos_control_keys_reach_input_queue(cx: &mut gpui::TestAppContext) {
        use gpui::EntityInputHandler;

        let cx = cx.add_empty_window();
        let state = cx.new(|_| AppState::new());
        let pane = cx.new(|cx| {
            let mut pane = TerminalPane::new(state, "chat".into(), cx);
            pane.chats.insert(
                "chat".into(),
                ChatTabs {
                    active: 0,
        ...
        // Match the event shape produced by the vendored macOS backend:
        // Enter and Tab carry key_char even though they are not printable.
        for (key, key_char, expected) in [
            ("enter", Some("\n"), b"\r".as_slice()),
            ("tab", Some("\t"), b"\t".as_slice()),
            ("shift-tab", None, b"\x1b[Z".as_slice()),
            ("backspace", None, b"\x7f".as_slice()),
            ("escape", None, b"\x1b".as_slice()),
            ("a", Some("a"), b"".as_slice()),
            ("space", Some(" "), b"".as_slice()),
            ("ctrl-c", Some("c"), b"\x03".as_slice()),
            ("cmd-k", Some("k"), b"".as_slice()),
        ] {
            cx.update(|window, cx| {
                pane.update(cx, |pane, cx| {
                    pane.replace_text_in_range(None, "echo test", window, cx);
                    ...
                    let tab = pane.tab_mut("chat", 1).unwrap();
                    assert_eq!(
                        tab.coalescer.take(),
                        [b"echo test".as_slice(), expected].concat(),
                        "{key}"
                    );
                    tab.flush_task = None;
                })
        ...
        cx.update(|window, cx| {
            pane.update(cx, |pane, cx| {
                pane.replace_and_mark_text_in_range(None, "ni", Some(2..2), window, cx);
                ...
                assert_eq!(pane.marked, "ni");
                assert!(pane.tab_mut("chat", 1).unwrap().coalescer.is_empty());
                pane.replace_text_in_range(None, "\u{4f60}", window, cx);
                assert_eq!(
                    pane.tab_mut("chat", 1).unwrap().coalescer.take(),
                    "\u{4f60}".as_bytes(),
                );
            });
        });
