...
pub struct EngineBootConfig {
    /// Data directory for the embedded engine (`~/.holt`).
...
/// Owns the backend connection. Ends at an [`RpcClient`] speaking the typed
/// RPC; the trait exists so teardown stays backend-owned.
#[async_trait]
trait EngineBackend: Send + Sync {
    fn client(&self) -> &RpcClient;
    fn mode(&self) -> EngineMode;
    /// Graceful teardown (drains runs / flushes state for the backend).
...
/// Cheaply clonable handle to the backend.
#[derive(Clone)]
...
impl EngineHandle {
    /// Assemble the backend in-process and connect over the memory transport.
...
pub struct AppState {
    ...
    /// This engine's device id (from the EngineInfo attachment boundary;
    /// `None` until the engine connects — views degrade gracefully).
...
impl AppState {
    ...
    pub fn apply_spaces(&mut self, mut spaces: Vec<Space>) {
        ...
        // the first project in display order; its chats died with it, so a
        // matching chat selection is healed by the accompanying chats frame
        // (`apply_chats`).
    ...
    /// First project in display order — the canvas fallback when no (valid)
    /// selection exists.
    fn first_space_id(&self) -> Option<String> {
        self.spaces_sorted().first().map(|s| s.id.clone())
    }
    ...
    pub fn apply_chat_config(&mut self, chat_id: &str, config: holt_proto::ChatConfig) {
    ...
    /// (a queued command executes on this device even fully offline); any
    /// other chat degrades when the OS says offline or its own edge room is
    /// down.
    ...
    pub fn auth_user(&self) -> Option<&holt_proto::UserProfile> {
    ...
    pub fn apply_transcript_frame(
        ...
        holt_doc::apply_transcript_frame(&mut self.transcript, frame)?;
        if is_reset {
            self.transcript_replayed = true;
        }
        if let Some(chat_id) = self.selected_chat.as_deref()
    ...
    pub fn apply_transfers(&mut self, transfers: Vec<holt_proto::TransferProgress>) {
    ...
    fn reconcile_change_request_watches(&mut self, cx: &mut Context<Self>) {
        ...
        for target in targets {
            ...
            let task = spawn_change_request_watch(cx, handle.clone(), target.clone());
...
fn spawn_watch<T: DeserializeOwned + 'static>(
    ...
    cx.spawn(async move |this, cx| {
        // Resubscribe loop: these are the standing Sessions/Spaces
            ...
            while let Some(value) = rx.recv().await {
                ...
                let alive = this.update(cx, |state, cx| {
                    ...
                    if let Some(rows) = state.sub_transcripts.get_mut(&doc_id) {
                        if let Err(err) = holt_doc::apply_transcript_frame(rows, frame) {
...
mod tests {
    ...
    // itself derives everything through `holt_proto::view`.
    ...
    fn transfer_percent_tracks_snapshots_by_upload_id() {
        ...
        let frame = |id: &str, done, total| holt_proto::TransferProgress {
            upload_id: id.into(),
    ...
    #[test]
    fn a_ready_connection_is_always_a_ready_gate() {
        for auth in [
            None,
    ...
    fn project_labels_from_cwd() {
        assert_eq!(project_label(Some("/home/w/dev/holt")), "holt");
        assert_eq!(project_label(Some("/home/w/dev/holt/")), "holt");
    ...
    fn grouped_sidebar_preserves_recency_order() {
        ...
        let chats = [
            chat_with_cwd("a", 9, Some("/dev/holt")),
            chat_with_cwd("b", 8, Some("/dev/zed")),
            chat_with_cwd("c", 7, Some("/dev/holt")),
        ...
        assert_eq!(labels, ["holt", "zed", "No project"]);
        let holt_ids: Vec<&str> = groups[0].chats.iter().map(|c| c.id.as_str()).collect();
        assert_eq!(holt_ids, ["a", "c"]);
        assert!(group_chats(std::iter::empty()).is_empty());
    }

    #[test]
    fn relative_times_match_holt_format() {
        let now = Utc::now();
    ...
    fn chat_location_joins_project_and_branch() {
        ...
        c.branch = Some("holt/rebalance".into());
        assert_eq!(
            chat_location(&c).as_deref(),
            Some("soccertcg · holt/rebalance")
        );
        c.branch = None;
    ...
    fn delivery_degradation_and_queued_sends_tell_the_truth() {
        ...
        // OS offline: a non-local chat degrades; a locally-hosted chat NEVER
        // does (the queued command executes on this device even fully
        // offline).
