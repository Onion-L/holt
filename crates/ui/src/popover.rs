...
use crate::motion::{self, HOLT_PULSE};
...
/// The floating-menu surface (holt `.glass-surface` + `menuSurface`):
...
/// `bg-white/[0.08]` (holt's `data-[highlighted]` styling) — two selected-
...
/// Small uppercase section heading inside a floating menu (holt
...
/// Hairline divider between menu sections (holt `MenuSeparator`:
...
/// The search/text input frame at the top of a picker popover (holt
...
/// A bordered trailing menu section (holt picker action groups /
...
// ---------------------------------------------------------------------------
// Dialog primitives (holt dialog.tsx / sidebar dialogs.tsx)
// ---------------------------------------------------------------------------
...
/// `transition-colors` (holt dialogs.tsx). Caller adds id + click; `fade_key`
/// as in [`menu_row`].
...
/// Pulsing skeleton rows shown while a list loads (holt:
