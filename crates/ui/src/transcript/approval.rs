...
use holt_doc::{
    GateVerdict, MessagePart, PlanApprovalState, PlanApprovalVerdict, SessionMessageEntry,
    ToolGate, ToolGateState,
};
...
impl Transcript {
    ...
    pub(super) fn render_plan_approval_card(
        ...
        let editor_key = format!("plan-{plan_id}");
    ...
    fn toggle_plan_feedback(
        ...
        let editor_key = format!("plan-{plan_id}");
        if self.approval_notes.remove(&editor_key).is_some() {
            cx.notify();
            return;
        }
        let input = cx.new(|cx| {
    ...
    fn resolve_plan_verdict(
        ...
        self.approval_notes.remove(&format!("plan-{plan_id}"));
        cx.notify();
        let Some(engine) = self.state.read(cx).engine().cloned() else {
            return;
        };
...
mod tests {
    ...
    fn plan_card_affordances_feedback_and_render(cx: &mut gpui::TestAppContext) {
        ...
        transcript.update(cx, |this, _| {
            assert!(this.approval_notes.contains_key("plan-plan-1"));
        });
        cx.update(|window, cx| {
            transcript.update(cx, |this, cx| {
                this.toggle_plan_feedback("plan-1".into(), window, cx);
