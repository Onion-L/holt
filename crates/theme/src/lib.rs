//! Holt's source-neutral theme domain model.
//!
...
impl ThemeRegistry {
    ...
    pub fn resolve(&self, selection: &ThemeSelection, appearance: Appearance) -> &ThemeVariant {
        self.variant(selection.variant_id(appearance))
            ...
            .expect("the built-in registry always contains both Holt variants")
    }
...
mod tests {
    ...
    fn builtins_have_complete_provenance_and_no_validation_errors() {
        ...
        assert!(registry.variant("holt-light").is_some());
        assert!(registry.variant("holt-dark").is_some());
