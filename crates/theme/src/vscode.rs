...
pub struct ImportReport {
    ...
    /// Deterministic repairs made after mapping so the compiled Holt roles
...
fn convert(theme: NormalizedTheme, options: ImportOptions) -> Result<ImportResult> {
    ...
    report.fallbacks.push(
        "surface treatment inferred as opaque because VS Code palettes target solid workbench backgrounds; the user's Holt surface preference can override it"
            .into(),
    );
    macro_rules! apply {
    ...
    if let Some(candidate) = report.accent_candidates.first() {
        ...
        report
            .fallbacks
            .push("accent.* retained the Holt fallback; curate a native accent".into());
    }
    ...
    for key in candidate_keys {
        ...
        if minimum_contrast(candidate, backgrounds) >= minimum {
            ...
            if let Some(mapping) = report
                .mappings
                .iter_mut()
                .find(|mapping| mapping.holt_role == role)
            {
...
mod tests {
    ...
    fn hardens_ayu_style_low_contrast_workbench_roles_without_blocking_import() {
        ...
        assert!(imported.report.adjustments.iter().any(|adjustment| {
            adjustment.holt_role == "text" && adjustment.reason.contains("editor.foreground")
        }));
        assert!(
            imported
                .report
    ...
    fn hardens_and_reports_low_contrast_faint_text() {
        ...
        let adjustment = imported
            ...
            .adjustments
            .iter()
            .find(|adjustment| adjustment.holt_role == "textFaint")
        ...
        let mapping = imported
            ...
            .mappings
            .iter()
            .find(|mapping| mapping.holt_role == "textFaint")
            .unwrap();
    ...
    fn flattens_translucent_foundational_surfaces() {
        ...
        assert!(imported.report.adjustments.iter().any(|adjustment| {
            adjustment.holt_role == "background"
