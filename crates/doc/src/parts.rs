...
use holt_proto::{AgentEvent, SUBAGENT_INPUT_KEEP, ToolCall, ToolDiff, UserInputQuestion};
...
///
/// Semantics from holt `foldEventIntoParts`:
        ...
        AgentEvent::Subagent {
            ...
            let status = match event.as_ref() {
                AgentEvent::Done { status, .. } => Some(match status {
                    holt_proto::DoneStatus::Errored => SubagentStatus::Failed,
...
/// continuations (`continuation_id(root, i)`), matching `splitMessageEntry` in holt.
