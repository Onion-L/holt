            ...
            for j in 0..parts.len() {
                ...
                if is_tool && id_matches {
                    ...
                    let is_spawn = part
                        ...
                        .and_then(|j| serde_json::from_value::<holt_proto::ToolCall>(j).ok())
...
///
/// Port of holt's `DocSegmentWriter` diff discipline: called with the *folded* parts of the
...
mod tests {
    ...
    use holt_proto::{AgentEvent, ToolCall};
    ...
    fn update_subagent_chip_refuses_non_spawn_parts() {
        ...
        let doc = SessionDoc::init("c1").unwrap();
        let mut w = SegmentWriter::begin(&doc, "e1", "dev", 1).unwrap();
        let tool = |id: &str, call: holt_proto::ToolCall| MessagePart::Tool {
            id: id.into(),
            call,
            is_error: false,
            resolved: true,
        ...
        let parts = vec![
            tool(
                "toolu_bash",
                holt_proto::ToolCall::Exec {
                    command: "git clone …".into(),
                },
            ),
            tool(
                "toolu_spawn",
                holt_proto::ToolCall::Unknown {
