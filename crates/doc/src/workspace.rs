//! Workspace doc schema over `loro` — the per-org entity index that replaces holt's
...
//! sets from any device — matching holt's Mutate surface. Presence rides the room's
//! `EphemeralStore` under keys `presence/{deviceId}` (an online timestamp), replacing
//! holt's 15s heartbeat writes so liveness never grows the oplog.
//!
//! Timestamps are stored as epoch millis (the session-doc convention) and surface as
//! `chrono::DateTime<Utc>` through the `holt_proto` entity types.
...
impl WorkspaceDoc {
    ...
    pub fn set_chat_source_context(
        &self,
        chat_id: &str,
        context: &holt_proto::ConversationSourceContext,
    ) -> Result<bool, DocError> {
        let Some(row) = self.existing_row("chats", chat_id) else {
            return Ok(false);
        };
...
mod tests {
    ...
    fn conversation_source_context_round_trips_with_legacy_fields() {
        let ws = WorkspaceDoc::new();
        ws.upsert_chat(&chat("chat-1", "dev-a")).unwrap();
        let context = holt_proto::ConversationSourceContext {
