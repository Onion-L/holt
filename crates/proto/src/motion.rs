//! Loader motion — the pure math behind holt's loading indicators.
//!
//! These are the curves and constants the gpui viewport animates with
//! (`holt-ui/src/motion.rs`, `holt-ui/src/loaders.rs`), lifted here so any
...
/// Holt loader pulse period.
pub const HOLT_PULSE_MS: u64 = 2_400;
/// Gradient matrix spinner wave period.
pub const GRADIENT_SPIN_MS: u64 = 750;

/// Cells in the holt wave loader.
pub const HOLT_CELLS: usize = 5;
/// Side length of the gradient spinner matrix.
pub const MATRIX_SIDE: usize = 3;

/// Holt loader cells rest at this opacity between pulses.
...
/// Per-row tints of the gradient matrix spinner — holt's "sunrise" gradient
...
/// Holt loader cell opacity for a phase: 0.08 → 1 → 0.08.
pub fn pulse_opacity(phase: f32) -> f32 {
    PULSE_MIN_OPACITY + (1.0 - PULSE_MIN_OPACITY) * pulse_wave(phase)
}

/// Holt loader cell scale for a phase: 0.9 → 1 → 0.9.
...
/// ported from holt's `gradient-spin-pulse` keyframes: full at the cycle
