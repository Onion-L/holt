...
/// The embedded engine is local-only, so a ready connection is always a ready
/// gate; auth state is not consulted.
pub fn gate_phase(
...
/// Session-row sub-line, "project · branch" (holt `chatLocation`): the repo
...
pub fn tool_group_summary(tools: &[(crate::ToolCall, bool)]) -> String {
    ...
    // Capitalize the first segment only (holt's style).
...
/// read the same on every surface. `holt-ui` has the oklch→sRGB math.
pub mod dot {
...
pub enum CheckoutPlan {
    ...
    /// `CreateWorktree` off `base` on send (the engine mints a `holt/<name>`
