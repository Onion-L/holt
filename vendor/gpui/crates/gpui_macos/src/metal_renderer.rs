...
pub(crate) struct MetalRenderer {
    ...
    /// Last time `HOLT_GPU_STATS` logged.
...
impl MetalRenderer {
    ...
    /// `HOLT_GPU_STATS=1`: log GPU-side memory holdings every 10s so
    /// regressions show up in logs without Instruments.
    fn maybe_log_gpu_stats(&mut self) {
        use std::sync::OnceLock;
        static ENABLED: OnceLock<bool> = OnceLock::new();
        if !*ENABLED.get_or_init(|| {
            std::env::var_os("HOLT_GPU_STATS").is_some_and(|v| v != "0" && !v.is_empty())
        }) {
            return;
        }
        ...
        // stderr, not `log`: consumers (holt) run tracing without a log
